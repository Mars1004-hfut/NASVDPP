...
def parse_args():
    ...
    parser.add_argument('--layers', nargs='?', default='[20,10]',
...
class DeepNASVDPP:
    ...
    def __init__(self, num_users, num_items, args):
        ...
        self.n_hidden = eval(args.layers)  # todo
    ...
    def _create_inference(self):
        with tf.name_scope("inference"):
            ...
            # Deep NASVD++
            layer1 = tf.multiply(self.embedding_q, (self.embedding_p + self.coeff*self.embedding_y))
            ...
            # self.inner = tf.add_n([out_layer, self.bias_i])

            self.output = tf.add_n([out_layer, self.bias_u, self.bias_i, self.bias_global])   #fixme

    def _create_loss(self):
        with tf.name_scope("loss"):

            # self.lambda_embedding_q = self.lambda_embedding_p
            # self.lambda_embedding_y = self.lambda_embedding_p
            # self.lambda_bias_u = self.lambda_embedding_p
            # self.lambda_bias_i = self.lambda_embedding_p
            # self.lambda_W = self.lambda_embedding_p
...
def training(flag, model, dataset, epochs):
    ...
    # weight_path = 'PretrainNASVDPP/ml-100k-91/valid_rmse_0.8993.ckpt'
    # weight_path = 'best/ml-100k-91/NASVD++/pretrain/valid_rmse_0.9001.ckpt'
    weight_path = 'result/yelp-91/NASVD++/10/pretrain/valid_rmse_1.0294.ckpt'
    saver = tf.train.Saver([model.b, model.W, model.c1, model.embedding_P, model.embedding_Q, model.biases_u, model.biases_i, model.biases_global])
    ...
    with tf.Session() as sess:
        ...
        if flag != 0:
            ...
            p_b, p_W, p_c1, p_e_P, p_e_Q, p_b_u, p_b_i, p_b_g = sess.run([model.b, model.W, model.c1, model.embedding_P, model.embedding_Q, model.biases_u, model.biases_i, model.biases_global])
            model.b = tf.Variable(p_b, name='Bias_for_MLP', dtype=tf.float32, trainable=True)
            model.W = tf.Variable(p_W, name='Weights_for_MLP', dtype=tf.float32, trainable=True)
            model.c1 = tf.Variable(p_c1, name='c1', dtype=tf.float32, trainable=True)
            model.embedding_Y_ = model.c1
        ...
        for epoch_count in range(epochs):
            ...
            if epoch_count % model.verbose == 0:
                ...
                if(rmse_valid > last_valid): #if rmse have incresed then stop
                    ...
                    validCount = 0
                if(validCount > 2):
                    print(model.dataset_name + '/%d,,,,%d,,,,%d,,,,%d,,,,%d,,,,%d,,,,%.1f,,,,%d,,,,%d,,,,%d,,,,%d-%d.txt' % (mt.log10(model.lambda_embedding_p), mt.log10(model.lambda_embedding_q), mt.log10(model.lambda_embedding_y),
                                                         mt.log10(model.lambda_bias_u), mt.log10(model.lambda_bias_i),
                                                         mt.log10(model.lambda_W), model.beta, mt.log10(model.learning_rate), mt.log10(model.reg_W[0]), mt.log10(model.reg_W[1]), mt.log10(model.reg_W[2]),model.embedding_size) )
                    exit()
                ...
                print("Epoch %d [%.1fs + %.1fs]: valid_rmse = %.4f, test_rmse = %.4f, [%.1fs] train_loss = %.4f [%.1fs]" % (
                    epoch_count, batch_time, train_time, rmse_valid, rmse_test, eval_time, train_loss, loss_time))
                with open('logDPNA/%s/%d-%d,,,,%d,,,,%d,,,,%d,,,,%d,,,,%d,,,,%.1f,,,,%d,,,,%d,,,,%d,,,,%d' % ( model.dataset_name,model.embedding_size, mt.log10(model.lambda_embedding_p), mt.log10(model.lambda_embedding_q), mt.log10(model.lambda_embedding_y),
                                                         mt.log10(model.lambda_bias_u), mt.log10(model.lambda_bias_i),
                                                         mt.log10(model.lambda_W), model.beta, mt.log10(model.learning_rate), mt.log10(model.reg_W[0]), mt.log10(model.reg_W[1]), mt.log10(model.reg_W[2])) + model.dataset_name + '.txt', 'a') as flog:
...
def  training_batch(batch_index, model, sess, batches):
    for index in batch_index:
        ...
        feed_dict = {model.user_input: user_input[:, None], model.user_hist: user_hist, model.num_idx: num_idx[:, None],
                     model.item_input: item_input[:, None], model.labels: labels[:, None], model.is_train_phase: True}
...
if __name__=='__main__':
    ...
    logging.info("regs:%.8f, %.8f, %.8f, %.8f  beta:%.1f  learning_rate:%.4f  train_loss:%d  activation:%d" % (regs[0], regs[1], regs[2], regs[3], args.beta, args.lr, args.train_loss, args.activation))

    dataset = Dataset(args.path + args.dataset)
    model = DeepNASVDPP(dataset.num_users, dataset.num_items, args)
    model.build_graph()
